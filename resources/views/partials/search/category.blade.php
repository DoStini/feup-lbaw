<li id="category-{{$category->id}}" class="list-group-item cat-item @if(count($category->child_categories) > 0) dropend @endif">
    ...
    <a class=" dropdown-toggle dropdown-toggle-split icon-click category-dropdown-{{$level}}" id="navbarDropdown-{{$category->id}}" data-bs-toggle="dropdown" aria-expanded="false">
...
<script>
    ...
    $('#category-{{$category->id}}').on('click', function(e) {
        e.stopPropagation();
    });
